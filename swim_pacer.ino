...
<body>
        ...
        <div id="coach" class="page">
            ...
            <div class="control">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                    ...
                    <span id="initialDelayValue">2 seconds</span>
                </div>
                <input type="range" id="initialDelay" min="0" max="30" step="1" value="2" oninput="updateInitialDelay()">
            </div>
            <div class="control">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                    <label for="swimmerInterval">Delay between swimmers:</label>
                    <span id="swimmerIntervalValue">1 second</span>
                </div>
                <input type="range" id="swimmerInterval" min="1" max="20" step="1" value="1" oninput="updateSwimmerInterval()">
            </div>
    ...
    <script>
        ...
        function updateQueueDisplay() {
            ...
            if (workSetQueues[currentLane].length === 0) {
                ...
                workSetQueues[currentLane].forEach((workSet, index) => {
                    ...
                    const isCompleted = workSet.completed;
        ...
        function startQueue() {
            ...
            // Find the first non-completed work set in current lane's queue
            const nextWorkSet = workSetQueues[currentLane].find(set => !set.completed);
            if (!nextWorkSet) return; // No pending sets to start
            
            // Start the next non-completed work set
            activeWorkSets[currentLane] = nextWorkSet;
        ...
        function handleSetCompletion() {
            ...
            if (COMPLETION_MODE === 'REMOVE_SET') {
                ...
                if (completedSetIndex !== -1) {
                    ...
                    workSetQueues[currentLane][completedSetIndex].completed = true;
                    workSetQueues[currentLane][completedSetIndex].completedAt = new Date();
                }
            }
            ...
            const remainingSets = workSetQueues[currentLane].filter(set => !set.completed);
